...
## 2.6.1 Elementos del DNS
 ...
 La **base de datos del DNS** está organizada de manera jerárquica. Por ejemplo, para la traducción del nombre de host `jerez.dec.usc.es`, la jerarquía es la siguiente:
   - **Servidor de Primer Nivel**: Maneja el dominio de nivel superior (**TLD**-> *top-level-domain*), como `.es`.
...
**Servidores Autorizados y No Autorizados**: 
...
La **CDN** **replica los contenidos de sus clientes** en los
servidores CDN y **los mantiene actualizados**. La CDN proporciona un mecanismo para que el contenido
sea entregado por el servidor CDN que pueda hacerlo
**más rápidamente**. 

Cuando se indica al navegador del host de un usuario que extraiga un **vídeo** concreto, la CDN debe **interceptar** la solicitud para **determinar un clúster de servidores** de la CDN que resulte **adecuado** para ese cliente en ese preciso instante y **redirigir** la solicitud del cliente a un servidor situado en dicho clúster. 
...
Se puede pensar com que el **host central** contiene un listado de que ordenadores hay y qué archivos ofertan por eso es un directorio.
...
Podemos pensarlo como que cada usuario tiene fragmentos de 1 GB de una película pirata, y después si un usuario quiere verla, los junta en su ordenador.