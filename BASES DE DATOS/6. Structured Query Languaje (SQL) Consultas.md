...
```sql
select ALL | DISTINCT atrib1,..., atribN AS
from tabla
where condiciones
order by atrib1,...,atribN asc | desc
```

**where** operación de selección
**select** operación de proyección
...
# 6.2 Operaciones Básicas Adicionales en SQL

## 6.2.1 Operación de Renombrado
...
## 6.2.2 Operaciones con Cadenas de Caracteres
...
## 6.2.3 Orden en la Presentación de las Tuplas
...
# 6.3 Operaciones sobre conjuntos
...
GROUP BY, los únicos campos que podemos incluir en el SELECT sin que estén dentro
de una función de agregación, son los que vayan especificados en el **GROUP BY**.
...
## 6.7.2 Comparación de conjuntos
...
# 6.8 Expresiones de reunión en SQL

## 6.8.1Condiciones de reunión
